...
import { renderActiveForksUrl, makeOpenInPopup } from "../utils";

namespace ActiveForksdify {
  export const NAV_BTN_ID = "active-forks-btn-nav";
  export const NAV_BTN_CLASS = "active-forks-nav-btn";
  export const NAV_BTN_CLASS_SELECTOR = "." + NAV_BTN_CLASS;

  export const CSS_REF_BTN_CONTAINER = "active-forks-btn-container";
...
export class GitHubInjector extends InjectorBase {
  ...
  checkIsInjected(): boolean {
    const button = document.getElementById(`${ActiveForksdify.NAV_BTN_ID}`);
    const currentUrl = renderActiveForksUrl(this.config.activeForksUrl);
...
abstract class ButtonInjectorBase implements ButtonInjector {
  ...
  inject(currentUrl: string, openAsPopup: boolean) {
    ...
    const oldBtn = document.getElementById(ActiveForksdify.NAV_BTN_ID);
    if (oldBtn) {
    ...
    const detailsBtn = Array.from(actionbar.children).filter(
      (child) =>
        child.tagName.toLowerCase() === "details" &&
        child.id.endsWith("more-options-details")
    );
    if (
      btnGroup &&
      btnGroup.length > 0 &&
    ...
    if (primaryButtons && primaryButtons.length > 1) {
      Array.from(primaryButtons)
        ...
        .forEach((primaryButton) =>
          primaryButton.classList.replace("btn-primary", "btn-secondary")
        );
    }
  }

  protected renderButton(url: string, openAsPopup: boolean): HTMLElement {
    let classes = this.btnClasses + ` ${ActiveForksdify.NAV_BTN_CLASS}`;
    if (this.float) {
      classes = classes + ` float-right`;
    }

    const container = document.createElement("div");
    container.id = ActiveForksdify.CSS_REF_BTN_CONTAINER;
    container.className = classes;

    const a = document.createElement("a");
    a.id = ActiveForksdify.NAV_BTN_ID;
    a.title = "Active Forks";
    a.text = "Active Forks";
    a.href = url;
    a.target = "_blank";
    if (openAsPopup) {
